...
export const getSongFromSets = async (songSlug: string): Promise<PhishResponse<SetSong>> => {
  return fetchFn(`setlists/slug/${songSlug}.json`, { revalidate: 60 * 60 * 24 });
};

export const getVenueSetSongs = async (venueid: string): Promise<PhishResponse<SetSong>> => {
  console.log(venueid);
...
export const getSongDetail = async (songSlug: string): Promise<SongDetail> => {
  const resp = (await fetchFn(`songdata/slug/${songSlug}.json`)) as PhishResponse<SongDetail>;
  return resp.data[0];
};
